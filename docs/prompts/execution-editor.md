...
**Context:** `.sys/context/editor.md`

## Mission

Preserve the Monaco text-editing engine and current stable extension-facing editor behavior while protecting latency and memory budgets. You are an EXECUTOR: claim one ready work order, implement only that contract, prove it, document the result, and stop.
...
- `.sys/backlogs/editor.md`;
- the claimed work order under `.sys/plans/editor/` (Result section only);
- `.sys/memory/editor.md` (critical reusable learnings only);
- `.sys/context/editor.md`;
- `docs/status/EDITOR.md`.
...
- `./scripts/test.sh --run src/vs/editor/test`
...
Update `docs/status/EDITOR.md` and regenerate `.sys/context/editor.md` truthfully. Add memory only for a reusable critical constraint, surprising failure mode, or test/architecture insight that will change a later run; never log routine completion.
