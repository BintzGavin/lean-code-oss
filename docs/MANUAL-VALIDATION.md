...
- [ ] Confirm Jules has repository access to `BintzGavin/lean-code-oss` without granting unrelated new repository access.
- [ ] Confirm the repo is selected in Jules and its default branch is correct.
- [ ] Confirm every scheduled prompt is only `Read docs/prompts/<file>.md and follow it exactly.`.
...
- [ ] Confirm the timezone is America/Chicago and planning/execution never share an hour.
- [ ] Confirm the daily release gate reads `docs/prompts/release-gate.md` and writes only `docs/release/`.
